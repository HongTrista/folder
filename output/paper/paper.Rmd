...
abstract: The data collected by the city government used for analyzed the poll engagement is used in the paper. Residents' attitudes toward each topic and the changes of engagement rate through time being discussed. The data illustrates that half of the residents have issues with front yard parking and the response rate sharply declined from 0.8 to 0.5 in April 2020. The low response rate may cause non-response bias, which will cause the government to misunderstand the real feeling and thinking of residents.
...
A poll belongs to a human research survey, which is used to collect opinions from the population by the government. The report first wants to know which topic is the residents have the most problems with. The results show that the total number of applications for front yard parking is the highest among all topics,  and the proportion of the volume of applications that achieve the required response rate is the highest for front yard parking. The circumstance can infer that most people have problems with and focus on front yard parking. Meanwhile, the findings of the final result of each application show that the pass rate for front yard parking also the highest. The time-series graph illustrated a downward trend in both the number of total applications and applications about front yard parking. Combined the findings before, we can see the government is earnestly understanding the opinions of residents and is really helping everyone to solve their problems.
...
加一个front yard parking 的line chart， 显示出，政府有关心人们的问题并且同意人民的感受。无论从整体来看还是分支。。。。。。。
